...
function ic_product_listing_products( $archive_template, $multiple_settings ) {
	...
	if ( /* (!is_ic_only_main_cats() || is_search() || is_product_filters_active() || is_ic_ajax() ) && */ more_products() ) {
		...
		$notfound_filters_text = sprintf( __( 'Sorry, but nothing matched your search terms. Please try again with some different options or %sreset filters%s.', 'post-type-x' ), '<a href="' . esc_url( get_filters_bar_reset_url() ) . '">', '</a>' );
