	...
	static function theme_integration_wizard( $atts ) {
		...
		if ( is_ic_integration_wizard_page() ) {
			...
			if ( ! isset( $_GET['test_advanced'] ) ) {
				...
				$box_content .= '<p class="wp-core-ui"><a href="' . esc_url( add_query_arg( 'test_advanced', '1' ) ) . '" class="button-primary">' . __( 'Start Advanced Mode Test', 'post-type-x' ) . '</a>';
				if ( is_ic_shortcode_integration( null, false ) ) {
					$box_content .= '<a href="' . esc_url( add_query_arg( 'test_advanced', 'simple' ) ) . '" class="button-secondary">' . __( 'Use Shortcode Mode', 'post-type-x' ) . '</a></p>';
				} else if ( is_ic_simple_mode() ) {
					$box_content .= '<a href="' . esc_url( add_query_arg( 'test_advanced', 'simple' ) ) . '" class="button-secondary">' . __( 'Use Simple Mode', 'post-type-x' ) . '</a></p>';
				}
				...
				if ( is_ic_shortcode_integration( null, false ) ) {
					...
					$box_content .= '<p class="wp-core-ui" style="margin-top: 20px;"><a href="' . esc_url( add_query_arg( 'remove_shortcode_integration', '1' ) ) . '" class="button-primary">' . __( 'Remove the Shortcode and Start the Wizard', 'post-type-x' ) . '</a></p>';
				} else {
...
//$box_content .= '<p>' . __( 'Please use the buttons below to let the script know if the Automatic Advanced Integration is done right.', 'post-type-x' ) . '</p>';
					...
					if ( ! defined( 'AL_SIDEBAR_BASE_URL' ) ) {
						...
						$box_content .= implecode_settings_radio( __( 'Default Sidebar', 'post-type-x' ), 'default_sidebar', $integration_settings['default_sidebar'], array(
	...
	margin-left: -23px;}</style>';
					$box_content .= '<p class="wp-core-ui"><span class="ic_spinner"></span><a href="' . esc_url( add_query_arg( 'test_advanced', 'ok' ) ) . '" class="button-primary integration-ok">' . __( 'It\'s Fine', 'post-type-x' ) . '</a>';
					if ( is_ic_simple_mode() ) {
						$box_content .= '<a href="' . esc_url( add_query_arg( 'test_advanced', 'bad' ) ) . '" class="button-secondary">' . __( 'It\'s Broken', 'post-type-x' ) . '</a>';
					}
				...
				$box_content .= '<p>' . sprintf( __( 'If you are a developer or would like to have full control on the product pages templates, make sure to see the %stemplates docs%s.', 'post-type-x' ), '<a href="https://implecode.com/docs/ecommerce-product-catalog/product-page-template/#cam=advanced-mode&key=integration-advanced-success-page-template">', '</a>' ) . '</p>';
				...
				//$box_content .= '<a target="_blank" href="https://implecode.com/wordpress/product-catalog/theme-integration-guide/#cam=advanced-mode&key=integration-advanced-success" class="button-secondary">' . __( 'Free Theme Integration Guide', 'post-type-x' ) . '</a>';
				$box_content .= '</p>';
				...
				$box_content .= '<p class="wp-core-ui"><a href="' . admin_url( 'edit.php?post_type=al_product' ) . '" class="button-primary">' . __( 'Go to Admin', 'post-type-x' ) . '</a><a href="' . esc_url( add_query_arg( 'test_advanced', '1' ) ) . '" class="button-secondary">' . __( 'Restart Advanced Mode Test', 'post-type-x' ) . '</a></p>';
				self::enable_simple_mode();
				echo '<div id="integration_wizard" class="' . $class . '">' . implecode_success( $box_content, 0 ) . '</div>';
			}
		}
