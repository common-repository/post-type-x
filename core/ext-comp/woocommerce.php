...
class ic_catalog_woocommerce {
	...
	function content() {
		...
		if ( $submenu == 'woocommerce' ) {
			...
			$message .= '<br>';
			$message .= '<br>';
			$message .= sprintf( __( 'Please install free %1$s extension from %2$scatalog extensions menu%3$s.', 'post-type-x' ), 'WooCommerce Catalog', '<a href="' . admin_url( 'edit.php?post_type=al_product&page=extensions.php&tab=product-extensions' ) . '">', '</a>' );
			$message .= '<br>';
			$message .= '<br>';
			...
			$message .= '<br>';
			$message .= '<br>';
			$message .= sprintf( __( 'You can also %1$simport WooCommerce products into separate catalog%2$s.', 'post-type-x' ), '<a href="' . admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=product-settings&submenu=csv' ) . '">', '</a>' );
