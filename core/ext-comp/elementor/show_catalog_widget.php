	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'raw'   => '<br>' . __( 'Use this to position your catalog container. The catalog container will show different content for product listing, category page, single product page and search results.', 'plugin-name' ) . '<br><br>' . __( 'By default it shows the main catalog page content.', 'post-type-x' ) . '<br><br>' . sprintf( __( 'Go to %scatalog settings%s to adjust what is visible here.', 'post-type-x' ), '<a target="_blank" href="' . admin_url( 'edit.php?post_type=al_product&page=product-settings.php' ) . '">', '</a>' ) . '<br><br>' . sprintf( __( '%sHere%s you can edit all text that is displayed inside the catalog container.', 'post-type-x' ), '<a target="_blank" href="' . admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=names-settings&submenu=archive-names' ) . '">', '</a>' )
			]
		);
