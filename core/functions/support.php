	...
	function implecode_custom_support_settings_content() {
		...
		<?php if ( $submenu == 'support' ) { ?>
            <div class="setting-content submenu support-tab">
                ...
                <h2><?php _e( 'impleCode Support', 'post-type-x' ); ?></h2>
                <p><?php echo sprintf( __( '<b>%s is free to use</b>. That\'s great! It\'s a pleasure to serve it to you. Let\'s keep it free forever!', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME ); ?></p><?php /*
			  <p><?php _e('If you found eCommerce Product Catalog useful or it saved you some amount of time please consider to support its development by buying premium support or an extension. This is better than a donation, because you get even more value with it. All the income from premium support and extensions goes for eCommerce Product Catalog and its extensions development. Everybody wins.','ecommerce-product-catalog'); ?></p> */ ?>
                <p><?php __( 'This awesome plugin is being developed under impleCode brand, which is a legally operating European company. It means that you can be assured that the high-quality <b>development will be continuous</b>.', 'post-type-x' ) ?></p>
                <p><?php echo sprintf( __( 'For <b>free support</b>, please visit %ssupport forums%s where the plugin developers will give you valuable advice.', 'post-type-x' ), '<a href="https://wordpress.org/support/plugin/' . IC_CATALOG_PLUGIN_SLUG . '">', '</a>' ) ?></p>
                <p>
                    <strong><?php _e( 'Please consider upgrading to support the project.', 'post-type-x' ) ?></strong>
                </p>
                ...
                <div class="extension premium-support">
                    <a href="https://implecode.com/wordpress/plugins/premium-support/#cam=catalog-support-tab&key=support-link">
                        <h3><span><?php _e( 'Premium Toolset', 'post-type-x' ) ?></span></h3></a>
                    <p><?php _e( 'An upgrade that will add many valuable features to the catalog functionality!', 'post-type-x' ) ?></p>
                    <p><?php echo sprintf( __( 'One year of high quality and speedy %sPremium Support%s from the developers for just %s.', 'post-type-x' ), '<a href="https://implecode.com/wordpress/plugins/premium-support/#cam=catalog-support-tab&key=support-link">', '</a>', '$49' ) ?> </p>
                ...
                <div class="extension premium-support">
                    ...
                    <p><?php echo sprintf( __( '<b>Extensions apart of premium support</b> provide additional useful features. They improve %s in a field of <a href="%1$s">SEO</a>, <a href="%3$s">Productivity</a>, <a href="%2$s">Usability</a> and <a href="%4$s">Conversion</a>.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME, esc_url( 'https://implecode.com/wordpress/plugins/#seo_usability_boosters&cam=catalog-support-tab&key=extensions-link-seo' ), esc_url( 'https://implecode.com/wordpress/plugins/#seo_usability_boosters&cam=catalog-support-tab&key=extensions-link-usability' ), esc_url( 'https://implecode.com/wordpress/plugins/#productivity_boosters&cam=catalog-support-tab&key=extensions-link-productivity' ), esc_url( 'https://implecode.com/wordpress/plugins/#conversion_boosters&cam=catalog-support-tab&key=extensions-link-conversion' ) ) ?></p>
                    <p>
                ...
                <div style="clear: both; height: 10px;"></div>
                <h2><?php _e( 'Premium Toolset Features', 'post-type-x' ) ?></h2>
                <p><?php echo sprintf( __( 'Apart of fast, confidential email support <b>you will receive some advanced features</b> for %s.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME ); ?></p>
                <h4><?php _e( 'Premium Features:', 'post-type-x' ) ?></h4>
                <ol>
                    <li><strong><?php _e( 'Alternative Products', 'post-type-x' ) ?></strong>
                        - <?php _e( 'select alternative products for each product and display them in a separate tab or section', 'post-type-x' ) ?>
                        ;
                    </li>
                    <li><strong><?php _e( 'Alternative Products Table', 'post-type-x' ) ?></strong>
                        - <?php _e( 'display alternative products as a table with many different parameters to compare', 'post-type-x' ) ?>
                        ;
                    </li>
                    <li><strong><?php _e( 'Price Filter Ranges', 'post-type-x' ) ?></strong>
                        - <?php _e( 'set clickable price filter ranges in the price filter widget', 'post-type-x' ) ?>
                        ;
                    </li>
                    <li><strong><?php _e( 'Category template', 'post-type-x' ) ?></strong>
                        - <?php _e( 'select different template for categories', 'post-type-x' ) ?>;
                    </li>
                    <li><strong><?php _e( 'Disabled default image', 'post-type-x' ) ?></strong>
                        - <?php _e( 'disable default image for category pages', 'post-type-x' ) ?>;
                    </li>
                    <li><strong><?php _e( 'Disabled category name', 'post-type-x' ) ?></strong>
                        - <?php _e( 'disable category name - useful if your category image already has the name included', 'post-type-x' ) ?>
                        ;
                    </li>
                    <li><strong><?php _e( 'Automatic Product Descriptions', 'post-type-x' ) ?></strong>
                        - <?php _e( 'generate product descriptions from category descriptions', 'post-type-x' ) ?>
                        ;
                    </li>
                    <li><strong><?php _e( 'Enhanced category filter', 'post-type-x' ) ?></strong>
                        - <?php _e( 'only bottom category clickable or always show child categories', 'post-type-x' ) ?>
                        ;
                    </li>
                    <li><strong><?php _e( 'Checkbox category filter', 'post-type-x' ) ?></strong>
                        - <?php _e( 'checkbox instead of links for category filter', 'post-type-x' ) ?>;
                    </li>
                    <li><strong><?php _e( 'Alternative listing template', 'post-type-x' ) ?></strong>
                        - <?php _e( 'let the user switch the listing template on front-end - for example between grid or list.', 'post-type-x' ) ?>
                        ;
                    </li>
                    <li><strong><?php _e( 'Product Count widget', 'post-type-x' ) ?></strong>
                        - <?php _e( 'product count widget that shows the number of displayed products', 'post-type-x' ) ?>
                        ;
                    </li>
                    <li><strong><?php _e( 'Product Pagination widget', 'post-type-x' ) ?></strong>
                        - <?php _e( 'display pagination in any widget area', 'post-type-x' ) ?>;
                    </li>
                    <li><strong><?php _e( 'Product Promo widget', 'post-type-x' ) ?></strong>
                        - <?php _e( 'display selected or random product in widget area', 'post-type-x' ) ?>
                        ;
                    </li>
                    <li><strong><?php _e( 'Product tags', 'post-type-x' ) ?></strong>
                        - <?php _e( 'which is considered as SEO booster if used properly', 'post-type-x' ) ?>
                        ;
                    </li>
                    <li><strong><?php _e( 'Product tags filter', 'post-type-x' ) ?></strong>
                        - <?php _e( 'filter products by tags', 'post-type-x' ) ?>;
                    </li>
                    <li><strong><?php _e( 'Separate sidebar', 'post-type-x' ) ?></strong>
                        - <?php _e( 'on main listing, individual product, catalog search and category pages', 'post-type-x' ) ?>
                        ;
                    </li>
                    <li><strong><?php _e( 'Responsive sidebar', 'post-type-x' ) ?></strong>
                        - <?php _e( 'select styling and appearance for sidebar on small screens', 'post-type-x' ) ?>
                        ;
                    </li>
                    <li><strong><?php _e( 'More styling', 'post-type-x' ) ?></strong>
                        - <?php _e( 'set product sidebar and product description width', 'post-type-x' ) ?>
                        ;
                    </li>
                    <li><strong><?php _e( 'Category widget enhancement', 'post-type-x' ) ?></strong>
                        - <?php _e( 'Show child categories only on parent category pages', 'post-type-x' ) ?>
                        .
                    </li>
                </ol>
                <p><?php _e( 'Go ahead and use the Buy Now button above to receive the premium support service and the described features immediately. You will receive the premium extension on your PayPal email address immediately after the payment is confirmed.', 'post-type-x' ); ?></p>
                ...
                <p><?php echo sprintf( __( 'You can also choose one of the %scatalog extensions%s to get the premium support.', 'post-type-x' ), '<a href="https://implecode.com/wordpress/plugins/#cam=catalog-support-tab&key=extensions-link">', '</a>' ) ?></p>
                <h2><?php _e( 'Theme Integration', 'post-type-x' ) ?></h2>
