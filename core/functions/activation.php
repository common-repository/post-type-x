...
function ic_sample_long_desc() {
	...
	if ( ! is_ic_shortcode_integration() ) {
		...
		$long_desc .= '<p>' . sprintf( __( 'The simple mode allows using %s most features. You can build the product listing pages and category pages by using a %s shortcode. Simple mode uses your theme page layout so it can show unwanted elements on a product page. If it does please switch to Advanced Mode and see if it works out of the box.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME, '[show_products]' ) . '</p>';
		...
		if ( ! is_advanced_mode_forced( false ) ) {
			$long_desc .= '<p>' . sprintf( __( 'Click %shere%s to proceed.', 'post-type-x' ), '<a href="?test_advanced=1">', '</a>' ) . '</p>';
		} else {
