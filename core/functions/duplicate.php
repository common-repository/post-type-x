...
function ic_post_duplicator_notice() {
	...
	if ( ! empty( $ic_duplicated_id ) ) {
		...
		$link  = '<a href="' . get_edit_post_link( $ic_duplicated_id ) . '">' . __( 'here', 'post-type-x' ) . '</a>';
		$label = sprintf( __( '%1$s successfully duplicated! You can edit your new %2$s %3$s.', 'post-type-x' ), ic_ucfirst( $names['singular'] ), ic_lcfirst( $names['singular'] ), $link );
		?>
        <div class="updated">
            <p><?php echo $label; ?></p>
        </div>
