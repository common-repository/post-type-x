...
//if ( !empty( $first_option ) ) {
	...
	if ( $all_option ) {
		$select_box .= '<option class="id_all" name="' . $option_name . '[all]" value="all" ' . selected( 'all', $selected_value, 0 ) . '>' . __( 'All', 'post-type-x' ) . '</option>';
	}
	$select_box .= '</select>';
...
function show_product_order_dropdown( $archive_template = null, $multiple_settings = null, $instance = null ) {
	...
	echo '<form class="product_order ic_ajax ' . $class . '" data-ic_responsive_label="' . __( 'Sort by', 'post-type-x' ) . '" data-ic_ajax="product_order" action="' . $action . '"><select class="product_order_selector ic_self_submit" name="product_order">';
	if ( is_ic_product_search() ) {
		$selected = isset( $_GET['product_order'] ) ? esc_attr( $_GET['product_order'] ) : '';
		echo '<option value="" ' . selected( "", $selected, 0 ) . '>' . __( 'Sort by Relevance', 'post-type-x' ) . '</option>';
	}
...
function ic_catalog_filters_bar() {
	...
	if ( ! empty( $sidebar_content ) ) {
		...
		if ( ! empty( $reset_url ) ) {
			echo '<div class="reset-filters"><a href="' . esc_url( $reset_url ) . '">' . __( 'Reset Filters', 'post-type-x' ) . '</a></div>';
		}
	}
	$is_filter_bar = false;
	unset( $is_filter_bar );
...
function show_default_product_sort_bar( $archive_template, $multiple_settings = null ) {
	if ( get_option( 'old_sort_bar' ) == 1 ) {
		...
		if ( $show == 0 ) {
			...
			echo '<h3>' . __( 'Product Filters Bar has no widgets', 'post-type-x' ) . '</h3>';
			$current_url   = ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
			$customize_url = add_query_arg( array(
				'url'                           => urlencode( $current_url ),
				urlencode( 'autofocus[panel]' ) => 'widgets'
			), wp_customize_url() );
			echo sprintf( __( '%sAdd widgets to the filters bar now%s or %sdismiss this notice%s.', 'post-type-x' ), '<a href="' . $customize_url . '">', '</a>', '<a class="dismiss-empty-bar" href="#">', '</a>' );
			echo '</div>';
			echo '</div>';
			$is_filter_bar = false;
			unset( $is_filter_bar );
...
function ic_empty_list_text() {
	...
	$text  = sprintf( __( 'No %s available.', 'post-type-x' ), ic_strtolower( $names['plural'] ) );
