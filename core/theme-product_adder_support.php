...
class ic_catalog_notices extends ic_activation_wizard {
	...
	static function getting_started_docs_info() {
        ...
        <p class="bottom-container">
            <a rel="noopener" target="_blank" href="<?php echo esc_url( $getting_started_url ) ?>">
				<?php _e( 'Getting started guide with step by step instructions', 'post-type-x' ) ?>
            </a>
        </p>
		<?php
	}
	...
	function listing_page_info( $post ) {
		...
		if ( $listing_id == $post->ID ) {
			implecode_info( sprintf( __( 'This page is defined as the main catalog listing. You can change this in %scatalog settings%s.', 'post-type-x' ), '<a href="' . admin_url( 'edit.php?post_type=al_product&page=product-settings.php' ) . '">', '</a>' ) );
...
//$integration_type	 = get_integration_type();
	...
	function theme_check_notice() {
		...
		if ( is_integration_mode_selected() && get_integration_type() == 'simple' ) {
            ...
            <div class="notice notice-updated is-dismissible ic-notice" data-ic_dismissible="notice-ic-catalog-welcome">
                <div class="squeezer">
                    <h4><?php echo sprintf( __( 'You are currently using %s in Simple Mode. It is perfectly fine to use it this way, however some features are limited.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME ); ?></h4>
                    <h4><?php echo sprintf( __( 'To switch to Advanced Mode, please add %s to your product listing page.', 'post-type-x' ), ic_catalog_shortcode_name() ) ?></h4>
                    <h4><?php echo sprintf( __( 'You can also use awesome %sCatalog Me! theme%s.', 'post-type-x' ), '<a href="' . admin_url( 'theme-install.php?search=Catalog%20me' ) . '">', '</a>' ) ?></h4>
                    <p class="submit">
						<?php /* <a href="https://implecode.com/wordpress/product-catalog/theme-integration-guide/#cam=simple-mode&key=top-message" target="_blank" class="button-primary"><?php _e( 'Theme Integration Guide', 'post-type-x' ); ?></a> */ ?>
                        <a class="button-primary add-catalog-shortcode"><?php _e( 'Add Shortcode Now', 'post-type-x' ); ?></a>
                        <a class="skip button"
                           href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=product-settings&submenu=support' ) ?>"><?php _e( 'Plugin Support', 'post-type-x' ); ?></a>
                        <a class="skip button"
                           href="<?php echo esc_url( add_query_arg( 'hide_al_product_adder_support_check', 'true' ) ); ?>"><?php _e( 'I know, don\'t bug me', 'post-type-x' ); ?></a>
                    </p>
                </div>
            </div>
			...
			/* ?>
			  ...
			  <div class="squeezer">
			  <h4><?php _e( 'You are currently using eCommerce Product Catalog in Advanced Mode without the integration file. It is perfectly fine to use it this way, however the file may be very handy if you need more control over product pages. See the guide for quick integration file creation.', 'post-type-x' ); ?></h4>
			  ...
			  <div id="implecode_message" class="updated product-adder-message messages-connect">
			  <div class="squeezer">
			  <h4><?php echo sprintf( __( 'Congratulations! Now your theme is fully integrated with %s.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME ); ?></h4>
			  <p class="submit"><a href="<?php echo admin_url( 'post-new.php?post_type=al_product' ) ?>" class="button-primary"><?php _e( 'Add Product', 'post-type-x' ); ?></a> <a class="skip button" href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php' ) ?>"><?php _e( 'Product Settings', 'post-type-x' ); ?></a> <a href="https://implecode.com/docs/ecommerce-product-catalog/#cam=advanced-mode&key=top-message-docs" class="button"><?php _e( 'Help & Documentation', 'post-type-x' ); ?></a>
			  </p>
			  </div>
			  </div>
			 * */
            ...
            <div class="squeezer">
                <h4><?php echo sprintf( __( 'Thank you for choosing %1$s! If you have any questions or issues feel free to %2$spost a support ticket%3$s.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME, '<a href="https://implecode.com/support/#cam=simple-mode&key=support-top">', '</a>' ) ?></h4>
				<?php sprintf( __( '%s requires initial configuration in order to work properly &#8211; please click the Initial Configuration button to proceed.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME ) ?>
				<?php sprintf( __( '<strong>Your theme does not declare %s support</strong> &#8211; please proceed to sample product page where automatic layout adjustment can be made.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME ) ?>
				<?php sprintf( __( 'You can create your product listing page automatically or insert %s on any of your existing pages.', 'post-type-x' ), ic_catalog_shortcode_name() ) ?>
                <h4><?php echo sprintf( __( 'If you are looking for a great customizable product theme we recommend free %sCatalog Me! theme%s.', 'post-type-x' ), '<a href="' . admin_url( 'theme-install.php?search=Catalog+me%21' ) . '">', '</a>' ) ?></h4>
                <p class="submit">
					<?php
					...
					?>
                <h4><?php echo __( 'Your main catalog listing page has been created automatically with private status (it is not visible for visitors).', 'post-type-x' ) ?></h4>
			<?php
			$label = __( 'Publish Main Catalog Listing', 'post-type-x' );
			}
			?>
                <a class="button-primary"
                   href="https://implecode.com/docs/ecommerce-product-catalog/getting-started/#cam=default-mode&key=getting-started"><?php _e( 'Getting Started Guide', 'post-type-x' ) ?></a>
				<?php
				echo $this->create_listing_page_button( $label, true, 'button-secondary' );
				?>

				<?php // echo sample_product_button()                                ?>
				<?php /* <a href="https://implecode.com/docs/ecommerce-product-catalog/theme-integration-wizard/#cam=default-mode&key=top-message-video" class="button"><?php _e( 'Configuration Video', 'post-type-x' ); ?></a> */ ?>
				...
				<?php /* <a class="skip button" href="<?php echo esc_url( add_query_arg( 'hide_al_product_adder_support_check', 'true' ) ); ?>"><?php _e( 'Hide Forever', 'post-type-x' ); ?></a> */ ?>
                </p>
            </div>
            </div><?php
		}
	...
	function woocommerce_notice() {
		...
		if ( $this->show_woocommerce_notice() ) {
            ...
            <div class="notice notice-info is-dismissible ic-notice" data-ic_dismissible="notice-ic-woocommerce-compat">
                <h4><?php echo sprintf( __( 'Hey! It looks like you have %1$s installed. You can use %2$s with or without %1$s.', 'post-type-x' ), 'WooCommerce', IC_CATALOG_PLUGIN_NAME ) ?><?php echo ' ';
					_e( 'See the usage scenarios below:', 'post-type-x' ) ?></h4>
                <ul style="list-style: initial;list-style-position: inside;">
                    <li><?php echo sprintf( __( '%1$s and %2$s separately for different product catalogs', 'post-type-x' ), 'WooCommerce', IC_CATALOG_PLUGIN_NAME ) ?>
                        - <?php _e( "doesn't need any specific configuration", 'post-type-x' ) ?></li>
                    <li><?php echo sprintf( __( '%1$s as an alternative to %2$s', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME, 'WooCommerce' ) ?>
                        - <?php echo sprintf( __( 'go to %1$s %3$simport screen%4$s to transfer all %2$s products to %1$s and remove %2$s after that', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME, 'WooCommerce', '<a href="' . admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=product-settings&submenu=csv' ) . '">', '</a>' ) ?></li>
                    <li><?php echo sprintf( __( '%1$s with %2$s design', 'post-type-x' ), 'WooCommerce', IC_CATALOG_PLUGIN_NAME ) ?>
                        - <?php echo sprintf( __( 'install free %1$s plugin from %2$scatalog extensions menu%3$s', 'post-type-x' ), 'WooCommerce Catalog', '<a href="' . admin_url( 'edit.php?post_type=al_product&page=extensions.php&tab=product-extensions' ) . '">', '</a>' ) ?></li>
                </ul>
            </div>
			<?php
		}
	...
	static function simple_mode_notice() {
		...
		if ( $listing_status !== 'publish' ) {
			implecode_warning( sprintf( __( 'Disabled due to a lack of main catalog listing.%s', 'post-type-x' ), ic_catalog_notices::create_listing_page_button() ) );
		} else {
			implecode_warning( sprintf( __( 'Disabled in simple mode. Add %1$s to your %2$smain catalog listing page%3$s to enable it.', 'post-type-x' ), ic_catalog_shortcode_name(), '<a href="' . product_listing_url() . '">', '</a>' ) );
		}
	}
	...
	static function add_catalog_shortcode_script() {
        ...
        <script>jQuery(".add-catalog-shortcode").on("click", function (event) {
                ...
                jQuery.post(ajaxurl, data, function (response) {
                    jQuery('<a style="margin-left: 5px;" href="' + response + '" class="button-primary"><?php _e( 'See Your Product Listing', 'post-type-x' ); ?></a>').insertAfter(".add-catalog-shortcode");
                    jQuery(".add-catalog-shortcode").replaceWith("<?php _e( 'The listing has been added successfully!', 'post-type-x' ) ?>");
	...
	static function main_listing_button() {
		...
		if ( ! empty( $listing_url ) ) {
            ...
            <a href="<?php echo $listing_url ?>"
               class="button-secondary"><?php _e( 'See Main Catalog Listing', 'post-type-x' ); ?></a>
			<?php
		}
	}
...
//$links[] = '<a href="https://implecode.com/wordpress/plugins/premium-support/#cam=catalog-settings-link&key=support-link" target="_blank">Premium Support</a>';
	...
	function review_notice() {
		...
		/* ?>
		  <div class="update-nag implecode-review"><strong><?php _e( 'Rate this Plugin!', 'post-type-x' ) ?></strong> <?php echo sprintf( __( 'Please <a target="_blank" href="%s">rate</a> %s and tell me if it works for you or not. It really helps development.', 'post-type-x' ), 'https://wordpress.org/support/view/plugin-reviews/ecommerce-product-catalog#postform', 'eCommerce Product Catalog' ) ?> <span class="dashicons dashicons-no"></span></div> */
		$text = apply_filters( 'ic_review_notice_text', sprintf( __( '%s is free software. Would you mind taking <strong>5 seconds</strong> to <a target="_blank" href="%s">rate the plugin</a> for us, please? Your comments <strong>help others know what to expect</strong> when they install %s.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME, 'https://wordpress.org/support/plugin/' . IC_CATALOG_PLUGIN_SLUG . '/reviews/#new-post', IC_CATALOG_PLUGIN_NAME ) )
		?>
        <div class="notice notice-warning implecode-review is-dismissible">
            ...
            <p><a target="_blank"
                  ...
                  class="button-primary"><?php _e( 'Rate Now & Hide Forever', 'post-type-x' ); ?></a>
				<?php /* <a href="" class="button"><?php _e( 'Hide Forever', 'post-type-x' ); ?></a> */ ?>
        </div>
        <div class="update-nag notice notice-warning inline implecode-review-thanks"
             style="display: none"><?php echo sprintf( __( 'Thank you for <a target="_blank" href="%s">your rating</a>! We appreciate your time and input.', 'post-type-x' ), 'https://wordpress.org/support/view/plugin-reviews/ecommerce-product-catalog#new-post' ) ?>
        <span class="dashicons dashicons-yes"></span></div><?php
	}

	function translation_notice() {
		?>
        <div class="update-nag notice notice-warning inline implecode-translate"><?php echo sprintf( __( "<strong>Psst, it's less than 1 minute</strong> to add some translations to %s collaborative <a target='_blank' href='%s'>translation project</a>.", 'post-type-x' ), IC_CATALOG_PLUGIN_NAME, 'https://translate.wordpress.org/projects/wp-plugins/ecommerce-product-catalog', IC_CATALOG_PLUGIN_NAME ) ?>
