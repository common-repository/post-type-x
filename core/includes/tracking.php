...
// We send once a week (while tracking is allowed) to check in, which can be used to determine active sites
	...
	public function admin_notice() {
		...
		if (
			...
			echo sprintf( __( 'Allow %s to track plugin usage? Opt-in to tracking to help in plugin development. No sensitive data is tracked.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME );
			echo '&nbsp;<a href="' . esc_url( $optin_url ) . '" class="button-primary">' . __( 'Allow', 'post-type-x' ) . '</a>';
			echo '&nbsp;<a href="' . esc_url( $optout_url ) . '" class="button-secondary">' . __( 'Do not allow', 'post-type-x' ) . '</a>';
			echo '</p></div>';
		}
	}
...
jQuery(this).text("' . __( 'Processing...', 'post-type-x' ) . '");
...
jQuery(this).parent("p").next("p").find("textarea").show();
	...
	function confirm_deactivation_box() {
		...
		$box .= '<h3>' . __( 'If you have a moment, please let us know why you are deactivating', 'post-type-x' ) . ':</h3>';
		...
		$box .= '&nbsp;<a href="" class="submit_deactivate button-primary">' . __( 'Submit & Deactivate', 'post-type-x' ) . '</a>';
		$box .= '&nbsp;&nbsp;&nbsp;<a href="" class="button-secondary">' . __( 'Cancel', 'post-type-x' ) . '</a>';
		$box .= '&nbsp;&nbsp;&nbsp;<a href="" class="deactivate">' . __( 'Skip', 'post-type-x' ) . '</a>';
		$box .= '</div>';
		$box .= '</div>';
		$box .= '</div>';
	...
	function paused_plugin_report_email( $email, $url ) {
		if ( ! empty( $email['message'] ) && ( ic_string_contains( $email['message'], 'eCommerce Product Catalog' ) || ic_string_contains( $email['message'], 'post-type-x' ) ) ) {
