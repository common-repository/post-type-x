	...
	function set_product_messages( $messages ) {
		...
		if ( $quasi_post_type == 'al_product' ) {
			...
			$messages[ $post_type ] = array(
				...
				5  => isset( $_GET['revision'] ) ? sprintf( __( '%1$s restored to revision from %2$s', 'post-type-x' ), $singular, wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
				6  => sprintf( __( $singular . ' published. <a href="%1$s">View %2$s</a>', 'post-type-x' ), esc_url( get_permalink( $post_ID ) ), $singular ),
				7  => __( 'Page saved.' ),
				...
				9  => sprintf( __( '%3$s scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview %3$s</a>', 'post-type-x' ), date_i18n( __( 'M j, Y @ G:i' ), strtotime( $post->post_date ) ), esc_url( get_permalink( $post_ID ) ), $singular ),
