...
class product_category_filter extends WP_Widget {
		...
		if ( get_integration_type() != 'simple' ) {
			if ( ic_if_show_filter_widget( $instance ) ) {
				...
				if ( ! is_ic_ajax() || ( ! empty( $form ) || ! empty( $child_form ) ) ) {
					...
					echo '<div class="' . $class . ' ic_ajax" data-ic_responsive_label="' . __( 'Category', 'post-type-x' ) . '" data-ic_ajax="product-category-filter-container" data-ic_ajax_data="' . esc_attr( json_encode( array(
							'instance' => $instance,
							'args'     => $args
						) ) ) . '">';
					if ( ! empty( $form ) ) {
	...
	function form( $instance ) {
		if ( get_integration_type() != 'simple' ) {
            ...
            <p>
                <label
                        for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'post-type-x' ); ?>
            ...
            <p><input class="widefat" id="<?php echo $this->get_field_id( 'shortcode_support' ); ?>"
                      ...
                      value="1" <?php checked( 1, $instance['shortcode_support'] ) ?> /> <label
                        for="<?php echo $this->get_field_id( 'shortcode_support' ); ?>"><?php _e( 'Enable also for shortcodes', 'post-type-x' ); ?></label>
            </p>
			<?php
