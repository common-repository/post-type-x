...
class ic_epc_block_edit {
	...
	function edit_settings( $settings ) {
		...
		?>
        <h3><?php _e( 'Edit Mode', 'post-type-x' ); ?></h3>
        <table><?php
			implecode_settings_radio( __( 'Product Edit Mode', 'post-type-x' ), 'archive_multiple_settings[edit_mode]', $settings['edit_mode'], array(
				'classic' => __( 'Classic Editor', 'post-type-x' ),
				'blocks'  => __( 'Blocks', 'post-type-x' ) . ' (Gutenberg)',
				//'full_blocks' => __( 'Blocks for entire page', 'post-type-x' ) . ' (Gutenberg)',
			), 1, __( 'Choose how would you like to edit the products.', 'post-type-x' ) );
			?>
        </table>
		<?php
