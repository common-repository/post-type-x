...
class ic_sitewide_bar {
	...
	function settings( $design_schemes ) {
		?>
        <h3><?php _e( 'Sitewide Icons', 'post-type-x' ); ?></h3>
        <table>
			<?php
	...
	function customizer_sections( $wp_customize ) {
		$message         = __( 'The icons will appear in the main menu.', 'post-type-x' );
		$site_editor_url = admin_url( 'site-editor.php' );
		if ( ! empty( $site_editor_url ) ) {
			$message .= ' ';
			$message .= sprintf( __( 'You can also %sadd the Catalog Icons block to the menu%s if your theme supports site editing with blocks.', 'post-type-x' ), '<a href="' . esc_url( $site_editor_url ) . '">', '</a>' );
		}
