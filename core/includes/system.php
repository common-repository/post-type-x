	...
	if ( current_user_can( 'manage_product_settings' ) ) {
		if ( isset( $_GET['reset_product_settings'] ) ) {
			if ( isset( $_GET['reset_product_settings_confirm'] ) && check_admin_referer( 'ic_reset_product_settings_confirm' ) ) {
				...
				echo '<a class="button" href="' . esc_url( $confirm_reset_url ) . '">' . __( 'Yes', 'post-type-x' ) . '</a> <a class="button" href="' . esc_url( remove_query_arg( 'reset_product_settings' ) ) . '">' . __( 'No', 'post-type-x' ) . '</a>';
			}
			...
			if ( isset( $_GET['delete_all_products_confirm'] ) && check_admin_referer( 'ic_delete_all_products_confirm' ) ) {
				...
				echo '<a class="button" href="' . esc_url( $delete_products_confirm_url ) . '">' . __( 'Yes', 'post-type-x' ) . '</a> <a class="button" href="' . esc_url( remove_query_arg( 'delete_all_products' ) ) . '">' . __( 'No', 'post-type-x' ) . '</a>';
			}
			...
			if ( isset( $_GET['delete_all_product_categories_confirm'] ) && check_admin_referer( 'ic_delete_all_product_categories_confirm' ) ) {
				...
				echo '<a class="button" href="' . esc_url( $delete_categories_confirm_url ) . '">' . __( 'Yes', 'post-type-x' ) . '</a> <a class="button" href="' . esc_url( remove_query_arg( 'delete_all_product_categories' ) ) . '">' . __( 'No', 'post-type-x' ) . '</a>';
			}
			...
			if ( isset( $_GET['delete_old_filters_bar_confirm'] ) && check_admin_referer( 'ic_delete_old_filters_bar_confirm' ) ) {
				...
				echo '<a class="button" href="' . esc_url( $delete_old_filters_bar_confirm ) . '">' . __( 'OK', 'post-type-x' ) . '</a> <a class="button" href="' . esc_url( remove_query_arg( 'delete_old_filters_bar' ) ) . '">' . __( 'Cancel', 'post-type-x' ) . '</a>';
			}
		} else {
			?>
            <style>table.widefat {
            ...
            <table class="widefat" cellspacing="0" id="ic_tools">
                ...
                <tr>
                    ...
                    <td><a class="button"
                           href="<?php echo esc_url( add_query_arg( 'reset_product_settings', 1 ) ) ?>"><?php _e( 'Reset Catalog Settings', 'post-type-x' ) ?></a>
                    </td>
                </tr>
                <tr>
                    <td><?php _e( 'Delete all items', 'post-type-x' ); ?>:</td>
                    <td><a class="button"
                           href="<?php echo esc_url( add_query_arg( 'delete_all_products', 1 ) ) ?>"><?php _e( 'Delete all Catalog Items', 'post-type-x' ) ?></a>
                    </td>
                </tr>
                <tr>
                    <td><?php _e( 'Delete all categories', 'post-type-x' ); ?>:</td>
                    <td><a class="button"
                           href="<?php echo esc_url( add_query_arg( 'delete_all_product_categories', 1 ) ) ?>"><?php _e( 'Delete all Catalog Categories', 'post-type-x' ) ?></a>
                    </td>
                </tr>
				<?php
				if ( get_option( 'old_sort_bar' ) == 1 ) {
                    ...
                    <tr>
                        ...
                        <td><a class="button"
                               href="<?php echo esc_url( add_query_arg( 'delete_old_filters_bar', 1 ) ) ?>"><?php _e( 'Empty Default Filters Bar', 'post-type-x' ) ?></a>
                        </td>
                    </tr>
				<?php } ?>
                <tr>
                    <td><?php _e( 'Delete all items and categories on uninstall', 'post-type-x' ); ?>:
                    </td>
                ...
                <tr>
					...
					if ( ! empty( $done ) ) {
						...
						$reassign_data .= '<p>' . $done . ' Items Done! Still processing.</p>';
					}
					if ( ! empty( $info ) ) {
						$reassign_data .= '<p>' . $info . '</p>';
					}
					?>
                    <td><?php echo $reassign_data ?></td>
                </tr>
            ...
            <table class="widefat" cellspacing="0" id="status">
                ...
                <tr>
                    <td><?php echo sprintf( __( '%s Database Version', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME ); ?>
                        :
                    </td>
            ...
            <table class="widefat" cellspacing="0" id="status">
                <thead>
                <tr>
                    <th colspan="2"><?php _e( 'Active Plugins', 'post-type-x' ); ?>
	...
	if ( current_user_can( 'manage_product_settings' ) ) {
		if ( isset( $_POST['field'] ) ) {
			...
			if ( strpos( $checked, '|' ) !== false ) {
				...
				update_option( 'ic_' . $checked[0], $checked[1], false );
			}
		}
	}
	echo 'done';
