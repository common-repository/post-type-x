...
class ic_settings_search {
	...
	function search_html() {
        ...
        <form class="ic-settings-search<?php echo $class ?>">
            ...
            <input type="submit" class="button-primary" value="<?php _e( 'Search', 'post-type-x' ) ?>">
	...
	function search_results_settings( $search_word ) {
		if ( empty( $this->points ) ) {
			?>
            <div class="ic-settings-search-empty"><?php echo sprintf( __( "Didn't find any adjustment options for '%s'.", 'post-type-x' ), $search_word ) ?></div>
			<?php
			return;
		}
		?>
        <h2><?php echo sprintf( __( "%s adjustment options found in settings for '%s'", 'post-type-x' ), count( $this->points ), $search_word ) ?>
            :</h2>
        <ul>
			<?php
			...
			if ( count( $this->points ) > 5 ) {
				?>
                <li>
                    <div class="button-secondary ic-search-settings-show-button"><?php echo sprintf( __( 'Show %s more related adjustment options', 'post-type-x' ), count( $this->points ) - 5 ) ?></div>
                </li>
				<?php
			}
			?>
	...
	function search_results_docs( $search_word ) {
        ...
        <div class="ic-docs-ajax-search-container">
            <div class="ic-docs-search-placeholder"><?php _e( 'Searching also in docs...', 'post-type-x' ) ?></div>
        </div>
        <script>
            var data = {
	...
	function ajax_search_docs() {
		...
		if ( is_wp_error( $request ) ) {
			implecode_warning( __( 'An error ocurred while searching. Please try again. ', 'post-type-x' ) );
			wp_die();
		...
		if ( ! empty( $data ) && is_array( $data ) ) {
			...
			?>
            <h2><?php
				echo sprintf( __( "%s documentation pages found for '%s'", 'post-type-x' ), count( $data ), $search_word )
				?>
                :</h2>
            <ul>
				<?php
				...
				if ( count( $data ) > 5 ) {
					?>
                    <li>
                        <div class="button-secondary ic-search-docs-show-button"><?php echo sprintf( __( 'Show %s more related docs', 'post-type-x' ), count( $data ) - 5 ) ?></div>
                    </li>
					<?php
				}
				?>
			...
			ob_start();
			?>
            <div class="ic-docs-search-empty"><?php echo sprintf( __( "Didn't find any docs or tutorials for '%s'.", 'post-type-x' ), $search_word ) ?></div>
			<?php
