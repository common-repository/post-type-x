...
function ic_admin_add_import_tab() {
    ...
    <a id="csv-settings" class="nav-tab<?php echo $class ?>"
       href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=product-settings&submenu=csv' ) ?>"><?php _e( 'Import / Export', 'post-type-x' ); ?></a>
	<?php
}
...
function implecode_custom_csv_settings_content() {
	...
	<?php if ( $submenu == 'csv' ) { ?>
        <div class="setting-content submenu csv-tab">
            ...
            <h2><?php
				_e( 'Simple CSV', 'post-type-x' );
				?>
            </h2>
            <h3><?php _e( 'Simple Export', 'post-type-x' ); ?></h3>
			<?php
			...
			if ( $export == 1 ) {
                ...
                <a style="display: block; margin-top: 20px;"
                   href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=product-settings&submenu=csv&export_csv=1' ) ?>">
                    <button class="button simple-export-button"><?php _e( "Export all items to CSV file", 'post-type-x' ) ?></button>
                </a>
                <h3><?php _e( 'Simple Import', 'post-type-x' ); ?></h3>
				<?php
...
function simple_upload_csv_products_file() {
	...
	if ( isset( $_FILES['product_csv'] ) && ( $_FILES['product_csv']['size'] > 0 ) ) {
		...
		echo '<form method="POST" enctype="multipart/form-data"><input type="file" accept=".csv" name="product_csv" id="product_csv" /><input type="submit" class="button" value="' . esc_attr( __( 'Import Now', 'post-type-x' ) ) . '" /></form>';
...
function simple_import_product_from_csv() {
	...
	if ( $fp !== false ) {
		...
		if ( count( $csv_cols ) == count( $import_array ) ) {
			...
			echo '<p>' . sprintf( __( '%s products successfully added to the catalog', 'post-type-x' ), $i ) . '.<p>';
			if ( ! empty( $error ) ) {
				echo '<p>' . sprintf( __( '%s failures occurred. Please check if the file is UTF-8 encoded', 'post-type-x' ), $error ) . '.</p>';
			}
			echo '</div>';
		} else {
			//echo '<div class="al-box warning">';
			//_e( 'Number of fields in database and number of fields in CSV file do not match!', 'post-type-x' );
			...
			echo '<div class = "al-box warning">';
			echo '<p>' . __( 'Number of product fields and number of fields in CSV file do not match!', 'post-type-x' ) . '</p>';
			...
			echo '<p>' . __( 'Please make sure that only the expected columns exist in the import file and the correct CSV separator is set.', 'post-type-x' ) . '</p>';
			echo '</div>';
			//echo '</div>';
		}
	}
...
function simple_prepare_csv_file( $type = 'w', $file_path = '' ) {
	...
	$fp = fopen( $file_path, $type ) or die( implecode_warning( sprintf( __( 'Permission error. Please check WordPress uploads %sfolder permissions%s.', 'post-type-x' ), '<a href="https://codex.wordpress.org/Changing_File_Permissions">', '</a>' ), 0 ) );
...
function ic_simple_csv_provide_file( $file_path ) {
	if ( ! current_user_can( 'read_private_products' ) ) {
		echo implecode_warning( __( "You don't have permission to read the exported file.", 'post-type-x' ) );
...
class IC_EPC_import_post_type {
	...
	public function import_initial_html() {
		...
		if ( ! empty( $post_types_dropdown ) ) {
			echo '<h3>' . __( 'Import from other content', 'post-type-x' ) . '</h3>';
			echo '<form>';
			foreach ( $_GET as $key => $value ) {
				echo '<input type="hidden" name="' . esc_attr( $key ) . '" value="' . esc_attr( $value ) . '">';
			}
			echo $post_types_dropdown . ' <button type="submit" class="button-secondary">' . __( 'Import', 'post-type-x' ) . '</button>';
			echo '</form>';
		}
	}
