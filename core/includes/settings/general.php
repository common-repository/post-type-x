    ...
    <div class="overall-product-settings settings-wrapper" style="clear:both;">
		...
		<?php if ( $submenu == 'general-settings' or $submenu == '' ) { ?>
            <div class="setting-content submenu">
                ...
                <h2 style="display:inline-block;vertical-align:middle;"><?php _e( 'General Settings', 'post-type-x' ); ?></h2>
                <a style="display: inline-block;vertical-align: middle;margin-left: 10px;" class="button-secondary"
                   href="<?php echo esc_url( admin_url( 'edit.php?post_type=al_product&page=implecode_welcome' ) ) ?>"><?php _e( 'Configuration Wizard', 'post-type-x' ) ?></a>
                <a style="display: inline-block;vertical-align: middle;margin-left: 10px;" class="button-secondary"
                   href="<?php echo esc_url( admin_url( 'edit.php?post_type=al_product&page=system.php&reset_product_settings=1' ) ) ?>"><?php _e( 'Reset Catalog Settings', 'post-type-x' ) ?></a>
                <form method="post" action="options.php">
					...
					if ( ! is_advanced_mode_forced() || ic_is_woo_template_available() || is_ic_shortcode_integration() ) {
                        ...
                        <div class="ic-important-settings">
							...
							/*
							  implecode_info(
							  '<p>' . __( 'Select a page to display your products.', 'post-type-x' ) . '</p>' .
							  '<p>' . __( 'This is the most important setting. The selected page slug will be included in individual product urls.', 'post-type-x' ) . '</p>' .
							  '<p>' . sprintf( __( 'Place %s on this page.', 'post-type-x' ), ic_catalog_shortcode_name() ) . '</p>'
							  );
							 *
							 */
                            ...
                            <span><span style="vertical-align: middle;" title="<?php echo $tip ?>"
                                        class="dashicons dashicons-editor-help ic_tip"></span><?php _e( 'Main Catalog Page', 'post-type-x' ); ?>: </span>
							<?php
							...
							if ( ! is_integraton_file_active() ) {
                                ...
                                <table>
									<?php
									$descriptions['simple']   = '<strong>' . __( 'Simple Mode', 'post-type-x' ) . '</strong> (' . __( 'recommended', 'post-type-x' ) . ') - ' . __( 'will use your theme page template with catalog custom styling applied.', 'post-type-x' );
									$descriptions['advanced'] = '<strong>' . __( 'Advanced Mode', 'post-type-x' ) . '</strong>' . ' - ' . __( 'fully customizable layout with the visual configuration wizard.', 'post-type-x' );
									$descriptions['theme']    = '<strong>' . __( 'Theme Mode', 'post-type-x' ) . '</strong>' . ' - ' . __( 'your theme default template files will be used to display catalog pages.', 'post-type-x' );
									implecode_settings_radio( __( 'Layout mode', 'post-type-x' ), 'ic-fake-empty-feature', 'simple', array(
										'advanced' => $descriptions['advanced'],
										'theme'    => $descriptions['theme'],
										'simple'   => $descriptions['simple']
									), 1, __( 'Simple mode is recommended. Choose Advanced Mode if you want more control over the layout or theme mode if your theme should control the layout. ', 'post-type-x' ), '<br>', "integration-mode-selection" );
									?>
                                </table>
							<?php
							}
							...
							/*
							  implecode_info(
							  '<p>' . sprintf( __( 'Place %s on the page where you would like the product list to be displayed.', 'post-type-x' ), ic_catalog_shortcode_name() ) . '</p>' .
							  '<p>' . __( 'Simple mode is recommended.', 'post-type-x' ) . '</p>'
							  );
							 *
							 */
							?>
                                <table>
									...
									$descriptions['advanced'] = '<strong>' . __( 'Advanced Mode', 'post-type-x' ) . '</strong>' . ' - ' . __( 'fully customizable layout with the visual configuration wizard.', 'post-type-x' );
									$descriptions['theme']    = '<strong>' . __( 'Theme Mode', 'post-type-x' ) . '</strong>' . ' - ' . __( 'your theme default template files will be used to display catalog pages.', 'post-type-x' );
									implecode_settings_radio( __( 'Layout mode', 'post-type-x' ), 'archive_multiple_settings[integration_type][' . $theme . ']', ic_catalog_theme_integration::get_real_integration_mode(), array(
										'advanced' => $descriptions['advanced'],
										'theme'    => $descriptions['theme'],
										'simple'   => $descriptions['simple']
									), 1, __( 'Simple mode is recommended. Choose Advanced Mode if you want more control over the layout or theme mode if your theme should control the layout. ', 'post-type-x' ), '<br>', "integration-mode-selection" );
                                ...
                                <div class="simple_mode_settings">
									<?php
									implecode_info( __( 'Use the options below to adjust the output.', 'post-type-x' ), 1, 0, false );
									/*
									  implecode_info(
									  '<p>' . __( 'In simple mode you will have to use shortcodes to display category pages. The advanced mode will display them for you automatically.', 'post-type-x' ) . '</p>' .
									  '<p>' . __( 'Simple mode will use your theme layout so it can show some unwanted elements on product catalog pages.', 'post-type-x' ) . '</p>' .
									  '<p>' . sprintf( __( 'If you want to switch to advanced mode easily, please insert %s on your product listing page.', 'post-type-x' ), ic_catalog_shortcode_name() ) . '</p>'
									  )
									 *
									 */
									?>
                                ...
                                <div class="advanced_mode_settings" style="display: none">
									<?php
									implecode_info(
										'<p>' . sprintf( __( 'In Advanced Mode %s must figure out your theme markup to display products properly.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME ) . '</p>' .
										...
										'<p>' . sprintf( __( 'If you have any problems with the integration, please insert %s on any page to display the catalog.', 'post-type-x' ), ic_catalog_shortcode_name() ) . '</p>'
									);
                                    ...
                                    <table class="advanced_mode_settings_hidden">
										...
										if ( ! defined( 'AL_SIDEBAR_BASE_URL' ) ) {
											implecode_settings_radio( __( 'Default Sidebar', 'post-type-x' ), 'archive_multiple_settings[default_sidebar]', $archive_multiple_settings['default_sidebar'], array(
                                ...
                                <div class="theme_mode_settings" style="display: none">
									...
									implecode_info(
										...
										'<p>' . sprintf( __( 'If you have access to the server files, you can also use our %sTheme Integration Guide%s to have full control over the layout.', 'post-type-x' ), '<a href="https://implecode.com/wordpress/product-catalog/theme-integration-guide/#cam=simple-mode&key=general-integration-info">', '</a>' ) . '</p>' .
										'<p>' . sprintf( __( 'If you have any problems with the integration, please insert %s on any page to display the catalog.', 'post-type-x' ), ic_catalog_shortcode_name() ) . '</p>'
									)
									?>
                                </div>
						...
						if ( ! empty( $url ) ) {
							$message = sprintf( __( 'Add %1$s to the %2$smain catalog page%3$s to have more styling options.', 'post-type-x' ), ic_catalog_shortcode_name(), '<a href="' . $url . '">', '</a>' );
						} else {
							$url     = admin_url( 'post-new.php?post_type=page' );
							$message = sprintf( __( '%1$sCreate a main catalog listing page%2$s with %3$s to have more styling options.', 'post-type-x' ), '<a href="' . $url . '">', '</a>', ic_catalog_shortcode_name() );
						}
					...
					if ( file_exists( AL_BASE_PATH . '/modules/cart/index.php' ) ) {
						?>
                        <h3><?php _e( 'Catalog Mode', 'post-type-x' ); ?></h3>
                        <table><?php
							implecode_settings_radio( __( 'Catalog Mode', 'post-type-x' ), 'archive_multiple_settings[catalog_mode]', $archive_multiple_settings['catalog_mode'], array(
							...
							), 1, __( 'Choose your usage scenario.', 'post-type-x' ) );
							?>
                        </table>
						<?php
					}
					?>
                    <h3><?php _e( 'Catalog Label', 'post-type-x' ); ?></h3>
                    <table><?php
                    ...
                    <table>
						...
						if ( empty( $integration_panel_shown ) ) {
                            ...
                            <tr>
                                <td>
                                    <span title="<?php _e( 'The page where the main product listing shows. Also this page slug will be included in product url.', 'post-type-x' ) ?>"
                                ...
                                <td><?php
									if ( $enable_product_listing == 1 ) {
										$listing_url = product_listing_url();
										ic_select_page( 'product_archive', __( 'Default', 'post-type-x' ), $product_archive, true, $listing_url );
									} else {
										ic_select_page( 'product_archive', __( 'Default', 'post-type-x' ), $product_archive, true );
									}
									?>
                                </td>
                            </tr> <?php
						}
						implecode_settings_number( __( 'Listing shows at most', 'post-type-x' ), 'archive_multiple_settings[archive_products_limit]', $archive_multiple_settings['archive_products_limit'], $item_name, 1, 1, __( 'You can also use shortcode with products_limit attribute to set this.', 'post-type-x' ), 1 );
						implecode_settings_radio( __( 'Main listing shows', 'post-type-x' ), 'archive_multiple_settings[product_listing_cats]', $archive_multiple_settings['product_listing_cats'], array(
							'off'              => $uc_item_name,
                    ...
                    <div class="advanced_mode_settings_inline">
                        ...
                        <table>
							<?php if ( is_ic_permalink_product_catalog() ) { ?>
                                <tr>
                                <td>
                                    <span title="<?php echo sprintf( __( 'By default, the category parent slug cannot be the same as for products. If you want them to be the same, please look for %s extension in the catalog extensions menu.', 'post-type-x' ), 'Smarter Product URLs' ) ?>"
                                          class="dashicons dashicons-editor-help ic_tip"></span>
									<?php _e( 'Categories Parent URL', 'post-type-x' ); ?>:
                                </td>
								<?php
								$site_url = site_url();
                                ...
                                <td class="longer">
									<?php echo $site_url ?>/<input type="text"
                                                                   ...
                                                                   title="<?php _e( 'Cannot be the same as product listing page slug', 'post-type-x' ) ?> (<?php echo get_product_slug() ?>)."
                                                                   id="category_archive_url"
                                                                   value="<?php echo urldecode( sanitize_title( $archive_multiple_settings['category_archive_url'] ) ); ?>"/>/<?php _e( 'category-name', 'post-type-x' ) ?>
                                    /
                                </td>
                                </tr><?php
							}
							implecode_settings_radio( __( 'Category Page shows', 'post-type-x' ), 'archive_multiple_settings[category_top_cats]', $archive_multiple_settings['category_top_cats'], array(
								'off'                => $uc_item_name,
							...
							implecode_settings_radio( __( 'Categories Display', 'post-type-x' ), 'archive_multiple_settings[cat_template]', $archive_multiple_settings['cat_template'], array(
								'template' => __( 'Template', 'post-type-x' ),
								'link'     => __( 'URLs', 'post-type-x' )
							), true, __( 'Template option will display categories with the same listing theme as products. Link option will show categories as simple URLs without image.', 'post-type-x' ) );
							...
							implecode_settings_radio( __( 'Show Related', 'post-type-x' ), 'archive_multiple_settings[related]', $archive_multiple_settings['related'], array(
								'products'   => $uc_item_name,
								'categories' => __( 'Categories', 'post-type-x' ),
								'none'       => __( 'Nothing', 'post-type-x' )
							), 1, __( 'The related products or categories will be shown on the bottom of product pages.', 'post-type-x' ) );
                        ...
                        <table>
							<?php
							implecode_settings_text( __( 'Archive SEO Title', 'post-type-x' ), 'archive_multiple_settings[seo_title]', $archive_multiple_settings['seo_title'], null, 1, null, __( 'The title tag for selected product listing page. If you are using separate SEO plugin you should set it there. E.g. in Yoast SEO look for it in Custom Post Types archive titles section.', 'post-type-x' ) );
							...
							implecode_settings_checkbox( __( 'Enable Structured Data', 'post-type-x' ), 'archive_multiple_settings[enable_structured_data]', $archive_multiple_settings['enable_structured_data'], 1, __( 'Enable to show structured data on each single product page. Test it with Google’s Structured Data Testing Tool. You can modify the output with the structured-data.php template file.', 'post-type-x' ) )
							?>
...
function ic_create_new_listing_page() {
	...
	if ( empty( $listing_id ) || $listing_id === 'noid' ) {
		$button = ' <a class="button button-small" style="vertical-align: middle;" href="' . admin_url( 'edit.php?post_type=al_product&page=product-settings.php&create_new_listing_page=1' ) . '">' . __( 'Create New', 'post-type-x' ) . '</a>';
	}
