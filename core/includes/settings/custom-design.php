...
function design_menu() {
	if ( current_user_can( 'manage_product_settings' ) ) {
        ...
        <a id="design-settings" class="nav-tab"
           href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=design-settings&submenu=archive-design' ) ?>"><?php _e( 'Catalog Design', 'post-type-x' ); ?></a>
		<?php
	}
}
        ...
        <div class="settings-submenu">
            <h3>
                <a id="archive-design" class="element current"
                   href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=design-settings&submenu=archive-design' ) ?>"><?php _e( 'Listing Design', 'post-type-x' ); ?></a>
                <a id="single-design" class="element"
                   href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=design-settings&submenu=single-design' ) ?>"><?php _e( 'Single Page Design', 'post-type-x' ); ?></a>
                <a id="design-schemes" class="element"
                   href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=design-settings&submenu=design-schemes' ) ?>"><?php _e( 'Design Schemes', 'post-type-x' ); ?></a>
    ...
    <table class="design-table">
        ...
        <tr id="default-theme">
            <td class="with-additional-styling theme-name">
                <label><input type="radio" name="archive_template"
                              value="default" <?php checked( 'default' == $archive_template ); ?>><?php _e( 'Modern Grid', 'post-type-x' ); ?>
                </label></td>
        ...
        <tr>
            <td class="additional-styling">
				...
				<?php
				ic_register_setting( __( 'Per row products (Modern Grid)', 'post-type-x' ), 'modern_grid_settings[per-row]' );
				ic_register_setting( __( 'Per row categories (Modern Grid)', 'post-type-x' ), 'modern_grid_settings[per-row-categories]' );
				_e( 'Per row', 'post-type-x' )
				...
				<?php _e( 'Per row', 'post-type-x' ) ?>: <input type="number" min="1" max="5" step="1"
                                                                class="number_box"
                                                                name="modern_grid_settings[per-row-categories]"
                                                                value="<?php echo $modern_grid_settings['per-row-categories'] ?>"><?php _e( 'categories', 'post-type-x' ); ?>
                <br>
				<?php
        ...
        <tr id="list-theme">
            <td class="with-additional-styling theme-name">
                <label><input type="radio" name="archive_template"
                              value="list" <?php checked( 'list' == $archive_template ); ?>><?php _e( 'Classic List', 'post-type-x' ); ?>
                </label></td>
        ...
        <tr id="grid-theme">
            <td class="with-additional-styling theme-name">
                <label><input type="radio" name="archive_template"
                              value="grid" <?php checked( 'grid' == $archive_template ); ?>><?php _e( 'Classic Grid', 'post-type-x' ); ?>
                </label></td>
        ...
        <tr>
            <td class="additional-styling">
                <strong><?php _e( 'Additional Settings', 'post-type-x' ); ?></strong><br>
				<?php
				ic_register_setting( __( 'Per row products (Classic Grid)', 'post-type-x' ), 'classic_grid_settings[entries]' );
				ic_register_setting( __( 'Per row categories (Classic Grid)', 'post-type-x' ), 'classic_grid_settings[per-row-categories]' );
				_e( 'Per row', 'post-type-x' )
				?>: <input type="number" min="1" step="1" class="number_box"
                           title="<?php _e( 'The product listing element width will adjust accordingly to your theme content width.', 'post-type-x' ) ?>"
                           name="classic_grid_settings[entries]"
                           value="<?php echo $classic_grid_settings['entries'] ?>"><?php echo $item_name ?><br>
				<?php _e( 'Per row', 'post-type-x' ) ?>: <input type="number" min="1" step="1"
                                                                class="number_box"
                                                                title="<?php _e( 'The product listing element width will adjust accordingly to your theme content width.', 'post-type-x' ) ?>"
                                                                name="classic_grid_settings[per-row-categories]"
                                                                value="<?php echo $classic_grid_settings['per-row-categories'] ?>"><?php _e( 'categories', 'post-type-x' ); ?>
                <br>
				<?php
...
function ic_product_page_design_settings() {
	...
	//implecode_upload_image( __( 'Default Image', 'post-type-x' ), 'default_product_thumbnail', default_product_thumbnail_url() );
	implecode_upload_image( __( 'Default Image', 'post-type-x' ), 'ic_default_product_image_id', ic_default_product_image_id(), default_product_thumbnail_url( true, false ), 'id' );
	?>
    ...
    <table>
		...
		implecode_settings_checkbox( __( 'Enable lightbox gallery', 'post-type-x' ), 'catalog_lightbox', $enable_catalog_lightbox, 1, __( 'The image on a single page will not be linked when unchecked.', 'post-type-x' ) );
		implecode_settings_checkbox( __( 'Enable image magnifier', 'post-type-x' ), 'catalog_magnifier', $enable_catalog_magnifier, 1, __( 'The image on a single page will be magnified when pointed with a mouse cursor.', 'post-type-x' ) );
	...
	if ( $submenu == 'design-schemes' ) {
        ...
        <form method="post" action="options.php">
            ...
            <div class="al-box info">
                <p><?php _e( "Changing design schemes has almost always impact on various elements. For example, changing price color has an impact on the single product page and archive page price color.", 'post-type-x' ); ?></p>
                <p><?php _e( "You can figure it out by checking the 'impact' column.", 'post-type-x' ); ?></p>
            </div>
                    ...
                    <td>
                        <select id="box_schemes" name="design_schemes[box-color]">
                            <option name="design_schemes[red-box]"
                                    value="red-box"<?php selected( 'red-box', $design_schemes['box-color'] ); ?>><?php _e( 'Red', 'post-type-x' ); ?></option>
                            <option name="design_schemes[orange-box]"
                                    value="orange-box"<?php selected( 'orange-box', $design_schemes['box-color'] ); ?>><?php _e( 'Orange', 'post-type-x' ); ?></option>
                            <option name="design_schemes[green-box]"
                                    value="green-box"<?php selected( 'green-box', $design_schemes['box-color'] ); ?>><?php _e( 'Green', 'post-type-x' ); ?></option>
                            <option name="design_schemes[blue-box]"
                                    value="blue-box"<?php selected( 'blue-box', $design_schemes['box-color'] ); ?>><?php _e( 'Blue', 'post-type-x' ); ?></option>
                            <option name="design_schemes[grey-box]"
                                    value="grey-box"<?php selected( 'grey-box', $design_schemes['box-color'] ); ?>><?php _e( 'Grey', 'post-type-x' ); ?></option>
                        </select>
                    </td>
                    <td>
                        <div class="product-name example <?php design_schemes( 'box' ); ?>">Exclusive Red Lamp</div>
                    </td>
                    <td><?php _e( 'product archive title', 'post-type-x' ); ?>
                        , <?php _e( 'archive pagination', 'post-type-x' ); ?></td>
                </tr>
                </tbody>
            </table>
