...
function register_product_extensions() {
	add_submenu_page( 'edit.php?post_type=al_product', __( 'Extensions', 'post-type-x' ), '<span class="extensions">' . __( 'Extensions', 'post-type-x' ) . '</span>', 'manage_product_settings', basename( __FILE__ ), 'product_extensions' );
}
...
function ic_epc_extensions_menu_elements() {
    ...
    <a id="extensions" class="nav-tab"
       href="<?php echo admin_url( 'edit.php?post_type=al_product&page=extensions.php&tab=product-extensions' ) ?>"><?php _e( 'Installation', 'post-type-x' ); ?></a>
	<?php
	/*
	  <a id="new-extensions" class="nav-tab" href="<?php echo admin_url( 'edit.php?post_type=al_product&page=extensions.php&tab=new-product-extensions' ) ?>"><?php _e( 'New', 'post-type-x' ); ?></a>
	 */
}
...
function product_extensions() {
    ...
    <div id="implecode_settings" class="wrap">
        <h1 class="wp-heading-inline"><?php echo sprintf( __( 'Extensions for %s', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME ) ?></h1>
		<?php do_action( 'ic_cat_extensions_page_start' ) ?>
        <hr class="wp-header-end">
        <h3><?php _e( 'All premium extensions come with premium support provided by the dev team.<br>Feel free to contact impleCode for configuration help, troubleshooting, installation assistance and any other plugin support at any time!', 'post-type-x' ) ?></h3>
        ...
        <div class="table-wrapper">
			...
			if ( $tab == 'product-extensions' or $tab == '' ) {
                ...
                <div class="helpers">
                    <div class="wrapper"><h2><?php _e( 'Did you Know?', 'post-type-x' ) ?></h2><?php
						text_helper( '', __( 'All extensions are designed to work with each other smoothly.', 'post-type-x' ) );
						text_helper( '', __( 'Some extensions give even more features when combined with another one.', 'post-type-x' ) );
						text_helper( '', __( 'Click on the extension to see full features list.', 'post-type-x' ) );
						text_helper( '', __( 'Paste your license key and click the install button to start the extension installation process.', 'post-type-x' ) );
						?>
                    </div>
                </div> <?php
                ...
                <div class="help">
					...
					?>
                    <h3><?php _e( 'Getting Started', 'post-type-x' ) ?></h3>
                    <ol>
                        <li>Go to the <a
                    ...
                    <h3><?php _e( 'How to get help?', 'post-type-x' ) ?></h3>
                    ...
                    <h3><?php _e( 'How to Install the extension?', 'post-type-x' ) ?></h3>
                    <ol>
                        <li>Go to the <a
                ...
                <div class="helpers">
                    <div class="wrapper"><h2><?php _e( 'Did you Know?', 'post-type-x' ) ?></h2><?php
						text_helper( '', __( 'The installation process takes less than 10 seconds.', 'post-type-x' ) );
						text_helper( '', sprintf( __( 'You can take advantage of premium support and <a href="%s">send support tickets</a> to impleCode developers once you have your license key.', 'post-type-x' ), 'https://implecode.com/support/?support_type=support' ) );
						?>
                    </div>
                </div>
				<?php
...
function ic_show_affiliate_content() {
	...
	if ( ! empty( $output ) ) {
		echo '<h2 class="partners-header">' . __( 'Fully compatible plugins from our partners', 'post-type-x' ) . '</h2>';
...
function extension_box( $extension, $all_ic_plugins, $not_active_ic_plugins ) {
	...
	if ( ! empty( $all_ic_plugins ) && is_ic_plugin_active( $slug, $all_ic_plugins ) ) {
		...
		$return      .= '<p><a ' . $disabled . ' href="' . wp_nonce_url( 'plugins.php?ic_cat_activation=1&action=activate&amp;plugin=' . urlencode( $plugin_file ), 'activate-plugin_' . $plugin_file ) . '" class="button-primary">Activate Now</a><span class="comp info">' . __( 'Installed Extension', 'post-type-x' ) . '</span></p>';
	} else {
		if ( $comp_class == 'wrong' ) {
...
function free_extension_box( $name, $url, $desc, $comp, $slug, $all_ic_plugins, $not_active_ic_plugins ) {
	...
	if ( ! empty( $all_ic_plugins ) && is_ic_plugin_active( $slug, $all_ic_plugins ) ) {
		...
		$return .= '<p><a ' . $disabled . ' href="' . wp_nonce_url( 'plugins.php?ic_cat_activation=1&action=activate&amp;plugin=' . urlencode( ic_cat_get_plugin_file( $slug ) ), 'activate-plugin_' . ic_cat_get_plugin_file( $slug ) ) . '" class="button-primary">Activate Now</a><span class="comp info">' . __( 'Installed Extension', 'post-type-x' ) . '</span></p>';
	} else {
		if ( $comp_class == 'wrong' ) {
	...
	if ( isset( $_GET['implecode_install'] ) && ! empty( $_GET['slug'] ) && ! empty( $_GET['license_key'] ) && current_user_can( 'install_plugins' ) ) {
		...
		if ( is_wp_error( $api ) ) {
			...
			echo '<div id="message error" class="error product-adder-message messages-connect">
				<div class="squeezer">
					<h4><strong>' . __( 'There is a problem with the connection. Please contact your server administrator with the following error:', 'post-type-x' ) . '</strong></h4>
						<p>' . $wp_errors . '</p>
				</div>
			</div>';
		} else if ( $api != 'error' ) {
			...
			echo '<div id="message error" class="error product-adder-message messages-connect">
				<div class="squeezer">
					<h4><strong>' . sprintf( __( 'This is not a valid license key! Get it <a href="%s">here</a>.', 'post-type-x' ), 'https://implecode.com/wordpress/plugins/' . esc_url( $_GET['url'] ) . '/#cam=extensions&key=' . esc_url( $_GET['url'] ) ) . '</strong></h4>
				</div>
			</div>';
		} else {
			echo '<div id="message error" class="error product-adder-message messages-connect">
				<div class="squeezer">
					<h4><strong>' . sprintf( __( 'The supplied license key is not valid for this extension! Upgrade it <a href="%s">here</a>.', 'post-type-x' ), 'https://implecode.com/wordpress/plugins/' . esc_url( $_GET['url'] ) . '/#cam=extensions&key=' . esc_url( $_GET['url'] ) ) . '</strong></h4>
				</div>
			</div>';
		}
		...
		echo '<div id="message error" class="error product-adder-message messages-connect">
				<div class="squeezer">
					<h4><strong>' . sprintf( __( 'You need to provide the license key to activate the extension. Get yours <a href="%s">here</a>.', 'post-type-x' ), 'https://implecode.com/wordpress/plugins/' . esc_url( $_GET['url'] ) . '/#cam=extensions&key=' . esc_url( $_GET['url'] ) ) . '</strong></h4>
				</div>
			</div>';
...
function add_product_catalog_upgrade_url() {
	if ( ! function_exists( 'start_implecode_updater' ) && false === get_site_transient( 'implecode_hide_extensions_box' ) ) {
		echo '<a target="_blank" title="' . __( 'Now you can get multiple extensions at once with the lowest price ever.', 'post-type-x' ) . '" class="upgrade-now" href="https://implecode.com/choose-a-plan/#cam=bundles&key=settings-top-menu">' . __( 'Upgrade Now!', 'post-type-x' ) . '</a>';
	}
}
...
function add_product_catalog_bundle_url() {
	if ( ! function_exists( 'start_implecode_updater' ) ) {
		//echo '<a target="_blank" title="' . __( 'Now you can get multiple extensions at once with the lowest price ever.', 'post-type-x' ) . '" class="upgrade-now" href="https://implecode.com/choose-a-plan/#cam=bundles&key=extensions-top-menu">' . __( 'Now extensions bundles from $19.99!', 'post-type-x' ) . '</a>';
	}
}
...
function extensions_bundle_box() {
	if ( ! function_exists( 'start_implecode_updater' ) ) {
		echo '<div class="bundle-box">' . __( 'Do you need multiple extensions?', 'post-type-x' ) . ' <a href="https://implecode.com/choose-a-plan/#cam=bundles&key=extensions-bundle-box">' . __( 'Check out extensions bundles', 'post-type-x' ) . '</a></div>';
	}
}
...
function ic_cat_extension_activated_message() {
	if ( ! empty( $_GET['ic_cat_extension_activated'] ) ) {
        ...
        <div id="message" class="updated notice is-dismissible">
            <p><?php _e( 'Extension <strong>activated</strong>.', 'post-type-x' ) ?></p></div>
		<?php
	}
}
